...
def get_irsu(batch):
    items_t, ratings_t, sizes_t, users_t, state_t = (
        batch["items"],
        batch["ratings"],
        batch["sizes"],
        batch["users"],
    ...
    return items_t, ratings_t, sizes_t, users_t, state_t
...
def batch_contstate_discaction(
    ...
    items_t, ratings_t, sizes_t, users_t, states_t = get_irsu(batch)
    items_emb = states_t
    b_size = ratings_t.size(0)

    items = items_emb
    next_items = torch.cat((items_emb[1:, :],items_emb[0,:].view(1,items.shape[1]))) ## 맨처음거를 맨 마지막으로 넣어줌 순환느낌으로..
    # ratings = ratings_t[:, :-1]
    # next_ratings = ratings_t[:, 1:]

    state = items
    next_state = next_items
    action = items_t
    reward = ratings_t
    ##user를 구분짓기 위한 용도인듯
    done = torch.zeros(b_size)
    done[torch.cumsum(sizes_t, dim=0) - 1] = 1
...
def prepare_batch_static_size(
    batch, item_embeddings_tensor, frame_size=10, embed_batch=batch_tensor_embeddings, num_items=500
):
    item_t, ratings_t, sizes_t, users_t, states_t = [], [], [], [], []
    for i in range(len(batch)):
        item_t.append(batch[i]["items"])
        ratings_t.append(batch[i]["rates"])
        sizes_t.append(batch[i]["sizes"])
        users_t.append(batch[i]["users"])
        states_t.append(batch[i]["states"])
    ...
    item_t = torch.tensor(np.concatenate(item_t))
    users_t = torch.tensor(users_t)
    ##학습데이터가 자료형이 잘못되어 있을 수 있으므로 바꿔줌
    ratings_t = torch.tensor(np.concatenate(ratings_t)).float()
    states_t = torch.tensor(np.concatenate(states_t)).float()
    sizes_t = torch.tensor(sizes_t)
    batch = {"items": item_t, "users": users_t, "ratings": ratings_t, "sizes": sizes_t, "states": states_t}
